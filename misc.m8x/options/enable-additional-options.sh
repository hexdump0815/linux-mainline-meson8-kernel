...
for i in `cat /compile/doc/stable-xd/misc.m8x/options/additional-options-yes.txt`; do
  echo $i
  ./scripts/config -e $i
done

for i in `cat /compile/doc/stable-xd/misc.m8x/options/additional-options-mod.txt`; do
  echo $i
  ./scripts/config -m $i
done
